...
def  main():
  ...
  installer = "\installer\ChipSmasher Installer.exe"
   
  if demo:
    installer = "\installer\ChipSmasher Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\ChipSmasher manual.pdf" 
  ]
  ...
  ZIPNAME = "ChipSmasher-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "ChipSmasher-v" + FULLVERSIONSTR + "-win-demo.zip"
